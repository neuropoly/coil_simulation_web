...
def calc_field(arrays_list, axis_dict, nb_elem, coil_definition = 100):
    ...
    r = np.zeros((coil_definition, 3))

    dl = np.zeros((coil_definition, 3))
    dl_cross_r = np.zeros((coil_definition, 3))
    norm_r = np.zeros((coil_definition, 1))
        ...
        for b in range(y_len):
            for c in range(z_len):
                for i in range(coil_definition-1):
                    ...
                    r[i][0] = (x_matrix[a, b, c] - (0.5*(arrays_list[nb_elem][i][0]+arrays_list[nb_elem][i+1][0])))
                    r[i][1] = (y_matrix[a, b, c] - (0.5*(arrays_list[nb_elem][i][1]+arrays_list[nb_elem][i+1][1])))
                    r[i][2] = (z_matrix[a, b, c] - (0.5*(arrays_list[nb_elem][i][2]+arrays_list[nb_elem][i+1][2])))

                    """Computes vector dl along the coil in direction of flow of current
                    We use the middle point between each point of the coil's array"""
                    dl[i][0] = arrays_list[nb_elem][i+1][0]+arrays_list[nb_elem][i][0]
                    dl[i][1] = arrays_list[nb_elem][i+1][1]+arrays_list[nb_elem][i][1]
                    dl[i][2] = arrays_list[nb_elem][i+1][2]+arrays_list[nb_elem][i][2]

                """We compute vectors between final and initial points outside of the loop"""
                r[coil_definition-1][0] = (x_matrix[a, b, c] - (0.5*(arrays_list[nb_elem][coil_definition-1][0]+arrays_list[nb_elem][0][0])))
                r[coil_definition-1][1] = (y_matrix[a, b, c] - (0.5*(arrays_list[nb_elem][coil_definition-1][1]+arrays_list[nb_elem][0][1])))
                r[coil_definition-1][2] = (z_matrix[a, b, c] - (0.5*(arrays_list[nb_elem][coil_definition-1][2]+arrays_list[nb_elem][0][2])))

                dl[coil_definition-1][0] = arrays_list[nb_elem][coil_definition-1][0]+arrays_list[nb_elem][0][0]
                dl[coil_definition-1][1] = arrays_list[nb_elem][coil_definition-1][1]+arrays_list[nb_elem][0][1]
                dl[coil_definition-1][2] = arrays_list[nb_elem][coil_definition-1][2]+arrays_list[nb_elem][0][2]
